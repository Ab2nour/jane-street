...
class LineLogic:
    ...
    @staticmethod
    def check_logic_same_diff(same, dif):
        for key, neighbors in same.items():
            if len(dif[key].intersection(neighbors)) != 0:
                return False
        return True
    ...
    def create_generator(self, start, length):
        ...
        if self.check_logic_same_diff(same_links, diff_links):
            def gen():
                if False:
                    yield 1
            return gen()
